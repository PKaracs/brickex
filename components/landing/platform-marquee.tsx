"use client";

import { Marquee } from "@/components/ui/marquee";
import Image from "next/image";

const platforms = [
  { 
    name: "Instagram", 
    logo: "https://bnibtqjlriohmuacvjmf.supabase.co/storage/v1/object/public/objects/Facebook%20Logo%20Grey%20Jan%2024%202026%20(1).png"
  },
  { 
    name: "TikTok", 
    logo: "https://bnibtqjlriohmuacvjmf.supabase.co/storage/v1/object/public/objects/Facebook%20Logo%20Grey%20Jan%2024%202026%20(4).png"
  },
  { 
    name: "LinkedIn", 
    logo: "https://bnibtqjlriohmuacvjmf.supabase.co/storage/v1/object/public/objects/Facebook%20Logo%20Grey%20Jan%2024%202026%20(2).png"
  },
  { 
    name: "Tinder", 
    logo: "https://bnibtqjlriohmuacvjmf.supabase.co/storage/v1/object/public/objects/Facebook%20Logo%20Grey%20Jan%2024%202026.png"
  },
  { 
    name: "Facebook", 
    logo: "https://bnibtqjlriohmuacvjmf.supabase.co/storage/v1/object/public/objects/Facebook%20Logo%20Grey%20Jan%2024%202026%20(3).png"
  },
];

export default function PlatformMarquee() {
  return (
    <section className="relative py-12 sm:py-16">
      {/* Top separator */}
      <div className="absolute top-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-zinc-800/50 to-transparent" />
      
      <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="text-left mb-6">
          <p className="text-sm sm:text-base text-zinc-500 font-medium">
            People actually use this in real life, on platforms you already care about
          </p>
        </div>

        {/* Marquee */}
        <div className="relative mb-6">
          <Marquee className="max-w-full [--duration:30s]" pauseOnHover>
            {platforms.map((platform, idx) => (
              <div
                key={idx}
                className="mx-3 sm:mx-4 flex items-center justify-center h-24 sm:h-32"
              >
                <div className="relative h-20 sm:h-24 w-auto">
                  <Image
                    src={platform.logo}
                    alt={`${platform.name} logo`}
                    width={300}
                    height={96}
                    className="h-full w-auto object-contain opacity-50 hover:opacity-80 transition-opacity duration-300"
                  />
                </div>
              </div>
            ))}
          </Marquee>

          {/* Gradient edges */}
          <div className="pointer-events-none absolute inset-y-0 left-0 h-full w-20 sm:w-32 bg-gradient-to-r from-[#0c0c0c]" />
          <div className="pointer-events-none absolute inset-y-0 right-0 h-full w-20 sm:w-32 bg-gradient-to-l from-[#0c0c0c]" />
        </div>

        {/* Arrow and note */}
        <div className="relative flex items-start gap-3 mt-4">
          <svg
            width="30"
            height="30"
            viewBox="0 0 150 82"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
            className="rotate-[250deg] text-zinc-600 flex-shrink-0 translate-y-1"
          >
            <path
              fillRule="evenodd"
              clipRule="evenodd"
              d="M67.616 0.147978C57.1498 0.781251 46.3786 4.08834 38.3061 9.10762C31.9231 13.0949 25.4463 19.3807 20.0724 26.7689C15.5199 33.0312 11.249 40.1849 8.76149 45.6967C6.18015 51.4431 4.37323 57.0487 2.26123 65.7738C-0.226243 76.0234 -0.531333 79.0491 0.712402 80.8785C1.1348 81.5118 1.36949 81.6525 2.02656 81.6525H2.84789L3.01215 79.6589C3.15295 77.6887 3.36414 76.774 6.34441 66.0552C8.85535 56.9314 10.7796 51.7714 14.018 45.4152C19.5796 34.4854 27.9103 23.626 35.2554 17.8092C42.5066 12.0629 51.1658 8.5447 61.984 6.90288C63.3216 6.71525 66.1845 6.59798 69.8453 6.59798C76.1813 6.62143 78.9269 6.90288 83.8315 8.09907C93.8987 10.5852 100.352 14.3614 113.259 25.3382C115.817 27.5194 121.003 32.4918 121.003 32.7732C121.003 32.8436 120.346 32.7732 119.548 32.6091C117.389 32.1634 116.286 32.2103 115.136 32.8202C113.939 33.4534 113.024 34.9311 113.024 36.2211C113.024 38.1678 114.291 39.5985 117.131 40.8651C117.905 41.1934 121.918 43.1636 126.048 45.2276C134.332 49.3791 135.857 50.0358 139.494 51.0443C143.906 52.2874 146.511 51.9356 148.271 49.8951C149.82 48.0891 150.031 45.6498 148.858 43.2574C148.506 42.5772 147.027 40.4898 145.549 38.6134C142.404 34.6496 141.7 33.5472 140.433 30.592C139.236 27.8009 138.18 24.0951 136.843 18.0438C134.543 7.44234 134.684 7.95834 134.027 8.61507C133.229 9.41252 132.032 17.8796 132.032 22.7112C132.032 26.1122 132.29 28.1996 133.088 31.4832C133.393 32.7732 133.628 33.8287 133.604 33.8522C133.581 33.8756 132.76 32.8202 131.797 31.5067C127.503 25.69 116.826 16.3551 107.275 10.0927C99.4838 5.00307 92.6784 2.35271 83.4091 0.851615C78.3872 0.0307056 73.248 -0.180385 67.616 0.147978Z"
              fill="currentColor"
            />
          </svg>
          <p className="text-xs sm:text-sm text-zinc-600 italic">
            * Used by 10,000+ creators & influencers
          </p>
        </div>
      </div>
      
      {/* Bottom separator */}
      <div className="absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-zinc-800/50 to-transparent" />
    </section>
  );
}

