...
const CONTENT_OPTIONS: ContentOption[] = [
  { id: "dating", label: "Dating profiles", icon: <Heart className="w-4 h-4" /> },
  { id: "instagram", label: "Instagram / Social", icon: <Camera className="w-4 h-4" /> },
  { id: "business", label: "Business / LinkedIn", icon: <Briefcase className="w-4 h-4" /> },
  { id: "personal_brand", label: "Personal brand", icon: <Zap className="w-4 h-4" /> },
  { id: "just_for_fun", label: "Just for fun", icon: <PartyPopper className="w-4 h-4" /> },
];
...
const TESTIMONIALS = [
  {
    quote: "Used this for my Hinge profile. Now I get asked which yacht club I'm at.",
    name: "Jake M.",
    stars: 5,
    image: "https://bnibtqjlriohmuacvjmf.supabase.co/storage/v1/object/public/objects/Richflex%20(17).png",
  },
  {
    quote: "I finally have photos I'm confident posting. Saved me hours and a photoshoot.",
    name: "Sarah K.",
    stars: 5,
    image: "https://bnibtqjlriohmuacvjmf.supabase.co/storage/v1/object/public/objects/Richflex%20(12).png",
  },
  {
    quote: "Posted a Richflex pic. Got 3 DMs asking about my lifestyle. Best investment.",
    name: "Marcus R.",
    stars: 5,
...
const FAQ_ITEMS = [
  { q: "What happens after I subscribe?", a: "You get instant access to all luxury templates, objects, and locations. Upload a selfie and start creating stunning photos in seconds." },
  { q: "Can I cancel, downgrade, or upgrade anytime?", a: "Absolutely. Cancel with one click from your dashboard — no hidden fees, no hassle. Upgrade or downgrade whenever you want." },
  { q: "What if I don't like the results?", a: "We're constantly improving. If you're not happy, reach out to our support and we'll help you get the perfect shot." },
  { q: "Do I get support if I have issues?", a: "Yes! We offer priority support for all subscribers. Reach us via chat or email anytime." },
  { q: "Can anybody else see my photos?", a: "No. Your photos are private by default. Only you can see and download your creations." },
];
...
export default function WelcomeClient({ subscription, isPreview }: WelcomeClientProps) {
  ...
  const handleSaveAndContinue = useCallback(async () => {
    setIsSaving(true);
    try {
      if (isPreview) { toast.success("Preview — would save & go to explore"); setIsSaving(false); setStep(0); return; }
      // Save in background — don't block navigation
      const savePromise = (goal && contentTypes.length > 0 && creatorType)
        ? saveOnboardingProfile({ goal, contentTypes, creatorType }).then(() => {
            posthog.capture("onboarding_completed", { goal, content_types: contentTypes, creator_type: creatorType });
            seline.welcome.completed({ goal, content_types: contentTypes, creator_type: creatorType });
          })
        : skipOnboarding().then(() => {
            posthog.capture("onboarding_skipped", { step, reason: "skip_for_now" });
  ...
  const handleSkip = useCallback(async () => {
    setIsSaving(true);
    try {
      if (isPreview) { toast.success("Preview — would skip to explore"); setIsSaving(false); setStep(0); return; }
      // Save in background — don't block navigation
      const savePromise = (goal && contentTypes.length > 0 && creatorType)
        ? saveOnboardingProfile({ goal, contentTypes, creatorType })
        : skipOnboarding();
      savePromise.catch((err) => console.error("[onboarding skip] save failed:", err));
  ...
  const handleUpgrade = async (plan: "starter" | "pro") => {
    ...
    try {
      ...
      sessionStorage.setItem(SESSION_STORAGE_KEYS.META_PURCHASE_EVENT_ID, purchId);
      trackMetaEvent("InitiateCheckout", { content_type: "subscription", currency: "USD", value: cfg.price }, initId);
      posthog.capture("checkout_initiated", { plan, source: "onboarding_pricing", value: cfg.price });
      if (goal && contentTypes.length > 0 && creatorType) { await saveOnboardingProfile({ goal, contentTypes, creatorType }); } else { await skipOnboarding(); }
      const result = await authClient.checkout({ slug: cfg.slug, redirect: true });
      if (result?.data?.url) { window.location.href = result.data.url; } else { toast.error("Unable to start checkout."); setIsCheckoutLoading(false); }
    } catch { toast.error("Something went wrong."); setIsCheckoutLoading(false); }
  };
  ...
  const handleContentContinue = () => {
    seline.welcome.stepCompleted(1, "content_types", { content_types: contentTypes });
    goNext();
  };
    ...
    <div className="fixed inset-0 z-50 bg-black flex flex-col overflow-hidden">
      ...
      <div className="relative z-10 flex-shrink-0 px-4 pt-4 pb-2">
        <div className="max-w-lg mx-auto flex items-center gap-3">
          {step > 0 && (
            <button onClick={step === 3 ? goBack : goBack} className="p-1.5 -ml-1 text-neutral-500 hover:text-white transition-colors rounded-lg hover:bg-white/5" aria-label="Go back">
              <ArrowLeft className="w-5 h-5" />
            </button>
          )}
          <div className="flex-1 h-1.5 bg-neutral-800/80 rounded-full overflow-hidden backdrop-blur-sm">
            <motion.div className="h-full rounded-full bg-gradient-to-r from-white/90 to-neutral-400/80" initial={{ width: 0 }} animate={{ width: `${progress}%` }} transition={{ duration: 0.5, ease: "easeOut" }} />
          </div>
          {step === 3 && (
            <button onClick={handleSkip} disabled={isSaving} className="p-1.5 -mr-1 text-neutral-500 hover:text-white transition-colors rounded-lg hover:bg-white/5" aria-label="Skip">
      ...
      <div className="relative z-10 flex-1 overflow-y-auto overflow-x-hidden">
        <div className="max-w-lg mx-auto px-4 py-4 flex flex-col min-h-full">
          <AnimatePresence mode="wait" custom={direction}>
            ...
            {/* ==================== STEP 0: Goal ==================== */}
            {step === 0 && (
              <motion.div key="s0" custom={direction} variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3, ease: "easeInOut" }} className="flex-1 flex flex-col">
                <div className="bg-neutral-900/70 backdrop-blur-xl rounded-3xl border border-neutral-800/60 p-6 sm:p-8 shadow-2xl shadow-black/50 flex-1 flex flex-col">
                  <h1 className="text-2xl sm:text-3xl font-bold text-white mb-1.5 leading-tight">What do you want to get out of Richflex?</h1>
                  <p className="text-neutral-500 text-sm mb-7">Let&apos;s personalize your experience</p>

                  <div className="grid grid-cols-2 gap-3 flex-1 content-center">
                    {GOAL_OPTIONS.map((opt) => (
                      <MagicCard key={opt.id} className="rounded-2xl cursor-pointer" gradientColor="#1a1a1a" gradientOpacity={0.6} gradientFrom={opt.gradientFrom} gradientTo={opt.gradientTo}>
                        <button onClick={() => handleGoalSelect(opt.id)} className={`relative flex flex-col items-center justify-center gap-3 p-5 sm:p-6 w-full min-h-[140px] rounded-2xl border transition-all duration-200 ${goal === opt.id ? "border-white/25 bg-white/[0.03]" : "border-transparent hover:border-neutral-700/50"}`}>
                          <div className={`w-14 h-14 rounded-2xl flex items-center justify-center bg-gradient-to-br ${opt.gradientBg} text-white`}>
                            {opt.icon}
                          </div>
                          <span className="text-white text-sm font-medium text-center">{opt.label}</span>
                          {goal === opt.id && (
                            <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} className="absolute top-2.5 right-2.5 w-5 h-5 rounded-full bg-white flex items-center justify-center">
            ...
            {/* ==================== STEP 1: Content ==================== */}
            {step === 1 && (
              <motion.div key="s1" custom={direction} variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3, ease: "easeInOut" }} className="flex-1 flex flex-col">
                <div className="bg-neutral-900/70 backdrop-blur-xl rounded-3xl border border-neutral-800/60 p-6 sm:p-8 shadow-2xl shadow-black/50 flex-1 flex flex-col">
                  <h1 className="text-2xl sm:text-3xl font-bold text-white mb-1.5 leading-tight">What will you use Richflex for?</h1>
                  <p className="text-neutral-500 text-sm mb-7">Tap all that apply</p>

                  <div className="flex flex-col gap-2.5 flex-1 justify-center">
                    {CONTENT_OPTIONS.map((opt) => {
                      const selected = contentTypes.includes(opt.id);
                      return (
                        <button key={opt.id} onClick={() => toggleContentType(opt.id)} className={`flex items-center justify-between px-4 py-3.5 rounded-xl border transition-all duration-200 ${selected ? "bg-white/[0.04] border-white/20" : "bg-white/[0.02] border-neutral-800/60 hover:border-neutral-700/60 hover:bg-white/[0.03]"}`}>
                          <span className="text-white text-sm font-medium">{opt.label}</span>
                          <div className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all duration-200 ${selected ? "bg-white text-black scale-95" : "bg-neutral-800/80 text-neutral-400"}`}>
                            {selected ? <Check className="w-4 h-4" /> : opt.icon}
                          </div>
                        </button>
                      );
                    })}
                  ...
                  <div className="mt-6">
                    <button onClick={handleContentContinue} disabled={contentTypes.length === 0} className="w-full py-3.5 rounded-xl bg-white text-black font-semibold text-sm transition-all hover:bg-neutral-200 active:scale-[0.98] disabled:opacity-20 disabled:cursor-not-allowed">Continue</button>
                    <p className="text-center text-neutral-600 text-xs mt-3">Select at least one</p>
                  </div>
                </div>
              </motion.div>
            )}
            ...
            {/* ==================== STEP 2: Creator Type ==================== */}
            {step === 2 && (
              <motion.div key="s2" custom={direction} variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3, ease: "easeInOut" }} className="flex-1 flex flex-col">
                <div className="bg-neutral-900/70 backdrop-blur-xl rounded-3xl border border-neutral-800/60 p-6 sm:p-8 shadow-2xl shadow-black/50 flex-1 flex flex-col">
                  <h1 className="text-2xl sm:text-3xl font-bold text-white mb-1.5 leading-tight">Who do you want to create photos of?</h1>
                  <p className="text-neutral-500 text-sm mb-7">You can change this anytime</p>

                  <div className="flex flex-col gap-3 flex-1 justify-center">
                    {CREATOR_OPTIONS.map((opt) => {
                      const selected = creatorType === opt.id;
                      return (
                        <MagicCard key={opt.id} className="rounded-2xl cursor-pointer" gradientColor="#1a1a1a" gradientOpacity={0.5} gradientFrom={opt.gradientFrom} gradientTo={opt.gradientTo}>
                          <button onClick={() => handleCreatorSelect(opt.id)} className={`flex items-center gap-4 px-5 py-5 w-full rounded-2xl border transition-all duration-200 ${selected ? "border-white/20 bg-white/[0.02]" : "border-transparent hover:border-neutral-700/50"}`}>
                            <div className="flex-1 text-left">
                              <h3 className="text-white font-semibold text-base">{opt.label}</h3>
                              <p className="text-neutral-500 text-sm mt-0.5">{opt.description}</p>
                            </div>
                            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0 bg-gradient-to-br ${opt.gradientBg} text-white transition-all ${selected ? "scale-95 border border-white/15" : ""}`}>
                              {opt.icon}
                            </div>
                          </button>
                        </MagicCard>
            ...
            {/* ==================== STEP 3: Pricing ==================== */}
            {step === 3 && (
              <motion.div key="s3" custom={direction} variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3, ease: "easeInOut" }}>
                  ...
                  <div className="relative">
                    ...
                    <div className="text-center mb-6">
                      <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/[0.05] border border-white/[0.08] mb-4">
                        ...
                        <span className="text-[11px] text-neutral-300 font-medium">Trusted by 10,000+ creators</span>
                      </div>
                      <h1 className="text-3xl sm:text-4xl font-bold text-white leading-tight tracking-tight">
                        START CREATING
                      </h1>
                      <p className="text-neutral-500 text-sm mt-2">Same you. Higher status photos. No photoshoot.</p>
                    </div>
                ...
                <div className="mt-5 bg-neutral-900/60 backdrop-blur-xl rounded-3xl border border-neutral-800/50 p-6">
                  <div className="flex items-center justify-center gap-0.5 mb-1">
                    {[1,2,3,4,5].map((i) => <Star key={i} className="w-3.5 h-3.5 text-white fill-white" />)}
                  </div>
                  <p className="text-center text-neutral-500 text-xs mb-5">Loved by 10,000+ creators</p>
                  <div className="space-y-3">
                    {TESTIMONIALS.map((t, i) => (
                      <div key={i} className="bg-white/[0.02] rounded-xl border border-neutral-800/40 p-4">
                        <div className="flex items-center gap-0.5 mb-2">{Array.from({ length: t.stars }).map((_, s) => <Star key={s} className="w-3 h-3 text-white/70 fill-white/70" />)}</div>
                        <p className="text-neutral-300 text-sm leading-relaxed mb-3">&quot;{t.quote}&quot;</p>
                        <div className="flex items-center gap-2.5">
                          <div className="w-12 h-12 rounded-full overflow-hidden border border-neutral-700/50"><img src={t.image} alt={t.name} className="w-full h-full object-cover" /></div>
                          <span className="text-neutral-300 text-xs font-medium">{t.name}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="mt-4 bg-neutral-900/60 backdrop-blur-xl rounded-3xl border border-neutral-800/50 p-6 mb-8">
                  <h2 className="text-center text-base font-bold text-white mb-0.5">Got Questions?</h2>
                  <p className="text-center text-neutral-500 text-xs mb-5">We&apos;ve Got Answers</p>
                  <div className="space-y-0">
                    {FAQ_ITEMS.map((faq, i) => (
                      <div key={i} className="border-b border-neutral-800/40 last:border-0">
                        <button onClick={() => setExpandedFaq(expandedFaq === i ? null : i)} className="w-full flex items-center justify-between py-3.5 text-left">
                          <span className="text-neutral-200 text-sm pr-4">{faq.q}</span>
                          <ChevronDown className={`w-4 h-4 text-neutral-500 flex-shrink-0 transition-transform duration-200 ${expandedFaq === i ? "rotate-180" : ""}`} />
                        </button>
                        <AnimatePresence>
                          {expandedFaq === i && (
                            <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                              <p className="text-neutral-500 text-sm pb-4 leading-relaxed">{faq.a}</p>
                            </motion.div>
                          )}
                        </AnimatePresence>
                      </div>
                  ...
                  <div className="mt-6 pt-4 border-t border-neutral-800/30 text-center">
                    <p className="text-neutral-700 text-[10px] leading-relaxed">
                      ...
                      <a href="/legal/terms" className="underline hover:text-neutral-500">Terms of Service</a>{" "}and{" "}
                      <a href="/legal/privacy" className="underline hover:text-neutral-500">Privacy Policy</a>
                    </p>
                  </div>
                </div>
